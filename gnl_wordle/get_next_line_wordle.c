...
char	*get_next_line(int fd, t_wordle *wordle)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	string[fd] = ft_fill_line_gnl(fd, string[fd], wordle);
	if (string[fd] == NULL)
	{
		return (NULL);
	}
	output = ft_next_line_gnl(string[fd], wordle);
	if (!output)
	{
		return (NULL);
	}
	string[fd] = ft_remaining_line_gnl(string[fd], wordle);
	return (output);
}
...
char	*ft_fill_line_gnl(int fd, char *line, t_wordle *wordle)
{
	...
	buffer = (char *)arena_alloc(wordle->arena, (BUFFER_SIZE + 1) * sizeof(char));
	if (buffer == NULL)
		return (NULL);
	dimension = 1;
	while ((ft_strchr_gnl(line, '\n')) == NULL && dimension > 0)
	{
		...
		if (dimension == 0 && (line == NULL || line[0] == '\0'))
			break ;
		buffer[dimension] = '\0';
		line = ft_strjoin_gnl(line, buffer, wordle);
		if (line == NULL)
			break ;
	}
	return (line);
...
char	*ft_next_line_gnl(char *line, t_wordle *wordle)
{
	...
	while (line[i] != '\n' && line[i] != '\0')
		i++;
	newstring = (char *)arena_alloc(wordle->arena, sizeof(char) * (i + 2));
	if (!newstring)
		return (NULL);
	i = 0;
	while (line[i] != '\n' && line[i] != '\0')
...
char	*ft_remaining_line_gnl(char *line, t_wordle *wordle)
{
	...
	str = (char *)arena_alloc(wordle->arena,
